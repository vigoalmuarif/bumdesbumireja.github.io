...
class ModelDataProdukPembelian extends Model
{

    protected $table = "produk";
    protected $column_order = array(null, 'produkID', 'sku', 'nama', null);
    protected $column_search = array('produk.nama',  'produk.sku');
    ...
    private function _get_datatables_query($keyword)
    {
        if (strlen($keyword) == 0) {
            $this->dt = $this->db->table($this->table)
                ->select('produk.id as produkID, sku, nama');
        } else {
            $this->dt = $this->db->table($this->table)
                ->select('produk.id as produkID, sku, nama')
